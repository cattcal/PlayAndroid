...
public final class TimeDialog {
    ...
    public static final class Builder
        ...
        public Builder(FragmentActivity activity) {
            ...
            for (int i = 0; i <= 23; i++) {
                hourData.add((i < 10 ? "0" : "") + i + " " + getString(R.string.dialog_time_hour));
            }

            // 生产分钟
            ArrayList<String> minuteData = new ArrayList<>(60);
            for (int i = 0; i <= 59; i++) {
                minuteData.add((i < 10 ? "0" : "") + i + " " + getString(R.string.dialog_time_minute));
            }

            // 生产秒钟
            ArrayList<String> secondData = new ArrayList<>(60);
            for (int i = 0; i <= 59; i++) {
                secondData.add((i < 10 ? "0" : "") + i + " " + getString(R.string.dialog_time_second));
            }
        ...
        public void onClick(View v) {
            if (v == mConfirmView) {
                if (mListener != null) {
                    mListener.onSelected(getDialog(), mHourView.getSelectedItem(),
                            mMinuteView.getSelectedItem(), mSecondView.getSelectedItem());
                }
                dismiss();
    ...
    public interface OnListener {
         ...
         * @param second            秒钟
         */
        void onSelected(Dialog dialog, int hour, int minute, int second);
